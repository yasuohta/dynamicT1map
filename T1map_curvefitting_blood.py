...
def filetoconc(path):
    ...
    for i, s in enumerate(slices):
        ...
        for i in tqdm(range(img3d.shape[0])):  
...
def imagesave(path,A,alpha,B,beta,resi):
    ...
    np.savetxt(path+'LV_beta.csv', beta, delimiter=",")
    np.savetxt(path+'LV_r_squared.csv', resi, delimiter=",")
...
def conc_time_map(folder,parameter):
    ...
    for i, time in enumerate(conc_time):
            ...
            dicom_save(conc[:,:,i],1.0,0.5,os.path.join(resultpath,str(conc_time[i])+"min_"+folder+"_LVconc.dcm"),str(conc_time[0])+"min")
    ...
    plt.savefig(os.path.join(path,files[0].StudyDate+folder+"LVconc_figure.png")) 
...
for path in paths:
    ...
    for folder in execfolders:
        ...
        dicom_save(A, 1, 0.5, os.path.join(resultpath,folder+"_LV_A.dcm"), "A")
        dicom_save(B, 1, 0.5, os.path.join(resultpath,folder+"_LV_B.dcm"), "B")
        dicom_save(alpha, 0.2, 0.1, os.path.join(resultpath,folder+"_LV_aplha.dcm"), "alpha")
        dicom_save(beta, 50, 25, os.path.join(resultpath, folder+"_LV_beta.dcm"), "beta")
        dicom_save(resi, 1, 0.5, os.path.join(resultpath, folder+"_LV_r_squared.dcm"), "r_square")
